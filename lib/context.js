...
function createContext(vm) {
	...
	function repl() {
		...
		function compileRepl(source, fun) {
			try {
				var lexer 		  = createLexer(source),
						parser    	= createParser(lexer),
						ast       	= parser.parseTopLevel(),
						compiler  	= createCompiler(),
						result    	= compiler.compileRepl(ast, fun);
		...
		var task = vm.spawn(compileRepl(""));

		rl.setPrompt("jester> ");
		rl.prompt();

		rl.on("line", function(input) {
			task.frames[1].fn = compileRepl(input, task.frames[1].fn);
			vm.resumeTask(task);
			setTimeout(function() { rl.prompt(); }, 0);
		}).on("close", function() {
			process.exit(0);
		});
	}
